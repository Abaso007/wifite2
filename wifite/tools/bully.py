...
class Bully(Attack, Dependency):
    ...
    def parse_state(self, line):
        ...
        if re_pin_not_found := re.search(
            r".*\[Pixie-Dust\] WPS pin not found", line
        ):
            state = '{R}Failed: {O}Bully says "WPS pin not found"{W}'

        if re_running_pixiewps := re.search(
            r".*Running pixiewps with the information", line
        ):
