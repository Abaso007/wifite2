...
class Reaver(Attack, Dependency):
    ...
    def _run(self):
        ...
        with Airodump(channel=self.target.channel,
            ...
            if self.crack_result is None and self.reaver_proc.poll() is not None:
                raise Exception(
                    f'Reaver process stopped (exit code: {self.reaver_proc.poll()})'
                )


    def get_status(self):
        main_status = '' if self.pixie_dust else '({G}%s{W}) ' % self.progress
    ...
    def parse_state(self, stdout):
        ...
        if new_pins := set(re.findall(r'Trying pin "([0-9]+)"', stdout_diff)):
    ...
    def get_pin_psk_ssid(stdout):
        ...
        # Check for PIN.
        ''' [+] WPS pin:  11867722 '''
        if regex := re.search(r"WPS pin:\s*([0-9]+)", stdout, re.IGNORECASE):
            pin = regex[1]
        ...
        if ssid is None:
            ...
            if regex := re.search(
                r"Associated with [0-9A-F:]+ \(ESSID: (.*)\)", stdout
            ):
                ssid = regex[1]
...
'''
    ...
    assert (
        ssid == 'Test Router'
    ), f'ssid was {repr(ssid)}, should have been Test Router'
